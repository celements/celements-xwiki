...
public class AttachmentService implements IAttachmentServiceRole {
  ...
  public XWikiAttachment addAttachment(XWikiDocument doc, InputStream in, String filename,
    ...
    if (comment == null) {
      if (attachment.isImage(getContext())) {
        ...
        comment = getContext().getMessageTool().get("core.comment.uploadAttachmentComment", params);
      }
    }
  ...
  String getFileName(String fieldName, String fieldNamePrefix, FileUploadPlugin fileupload)
      throws XWikiException {
    String filenameField = FILENAME_FIELD_NAME + fieldName.substring(fieldNamePrefix.length());
