...
public abstract class AbstractGenericComponentManager extends DelegateComponentManager {
    ...
    if (componentManager == null) {
      ...
      if (componentManager == null) {
        throw new RuntimeException(
            "Key-based Proxy Component Managers should always have a Parent Component "
  ...
  public void unregisterComponent(Class<?> role, String roleHint) {
    ...
    // Note: Ideally if the Component Manager for the current key is empty we could remove it from
    // the list
    // of managed Component Managers. However there's currently no way to ask a Component Manager
    // for its
